    ...
    var body: some View {
        VStack {
            if course.exercisesIndex < exercises.count {
                VStack {
                    switch exercises[course.exercisesIndex]
                        .type
                    {
                    case .WordInsertion:
                        WordInsertion(
                            exercises[course.exercisesIndex]
                                as! WordInsertion,
                            index: $course.exercisesIndex)
                    case .Crossword:
                        Crossword(
                            exercises[course.exercisesIndex]
                                as! Crossword,
                    ...
                    case .Speaking:
                        SpeakingExercise(
                            ...
                            index: $course.exercisesIndex)
                    case .Reading:
                        Reading(exercises[course.exercisesIndex]
                                as! Reading,index:$course.exercisesIndex)
                         
                    case .WordInsertionWithAnswerOptions:
                        WordInsertionWithAnswerOptions(
                            exercises[course.exercisesIndex]
                                as! WordInsertionWithAnswerOptions,
                            index: $course.exercisesIndex)
                    /*
                         case .Listening:
                         ComingSoonView()
...
struct LessonView_Previews: PreviewProvider {
    static var previews: some View {
        Lesson(
            name: "First Lesson", complexity: .A1,
            exercises: [
                WordCards(
