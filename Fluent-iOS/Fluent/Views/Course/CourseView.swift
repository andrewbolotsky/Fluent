    ...
    var body: some View {
        if !course.newAction {
            VStack {
                Spacer()
                Spacer()
                Text("Your course ").bold().font(
                    .largeTitle
                ).lineLimit(1)
                Text("\(course.name)").bold().font(
                    .largeTitle
                ).lineLimit(1)
                Spacer()
                Button {
                ...
                .buttonStyle(GeneralButtonStyle()).font(
                    .title
                ).bold().onReceive(
                    course.$newAction,
                    perform: { value in
            ...
            if course.newLesson == nil {
                ...
                VStack {
                    course.newLesson!.environmentObject(
                        course)
                }.onAppear {
                    course.initNewLessonWithNewAction(
                        newAction: $course.newAction)
                }
            }
        }
    }
