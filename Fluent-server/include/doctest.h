...
// - stringification - the detection of "ostream& operator<<(ostream&, const Type&)" and StringMaker<>
...
namespace detail {
    namespace types {
        ...
        // Compiler intrinsics
        template <typename Type> struct is_enum { static DOCTEST_CONSTEXPR bool value = __is_enum(Type); };
        template <typename Type> struct underlying_type { using type = __underlying_type(Type); };

        template <typename Type> struct is_pointer : false_type { };
        template <typename Type> struct is_pointer<Type*> : true_type { };

        template <typename Type> struct is_array : false_type { };
        // NOLINTNEXTLINE(*-avoid-c-arrays)
        template <typename Type, size_t SIZE> struct is_array<Type[SIZE]> : true_type { };
#endif
    }
...
// MSVS 2015 :(
    ...
    template <typename Type>
    struct has_global_insertion_operator<Type, decltype(::operator<<(declval<std::ostream&>(), declval<const Type&>()), void())> : types::true_type { };

    template <typename Type, typename = void>
    struct has_insertion_operator { static DOCTEST_CONSTEXPR bool value = has_global_insertion_operator<Type>::value; };

    template <typename Type, bool global>
    struct insert_hack;

    template <typename Type>
    struct insert_hack<Type, true> {
        static void insert(std::ostream& os, const Type& t) { ::operator<<(os, t); }
    };

    template <typename Type>
    struct insert_hack<Type, false> {
        static void insert(std::ostream& os, const Type& t) { operator<<(os, t); }
    };

    template <typename Type>
    using insert_hack_t = insert_hack<Type, has_global_insertion_operator<Type>::value>;
#else
...
struct has_insertion_operator<T, decltype(operator<<(declval<std::ostream&>(), declval<const T&>()), void())> : types::true_type { };
    ...
    struct StringMakerBase {
        ...
        static String convert(const DOCTEST_REF_WRAP(T)) {
            ...
            static_assert(deferred_false<Type>::value, "No stringification detected for type Type. See string conversion manual");
#endif
            return "{?}";
        }
    };
    ...
    void filloss(std::ostream* stream, const T (&in)[N]) { // NOLINT(*-avoid-c-arrays)
        // Type[N], Type(&)[N], Type(&&)[N] have same behaviour.
...
#else
    String ret = __PRETTY_FUNCTION__; // doctest::String toString() [with Type = TYPE]
...
#if defined(_MSC_VER) && _MSC_VER <= 1900
        insert_hack_t<Type>::insert(*stream, in);
#else
        operator<<(*stream, in);
#endif
        }
...
namespace detail {
    // clang-format off
#ifdef DOCTEST_CONFIG_TREAT_CHAR_STAR_AS_STRING
    template<class Type>               struct decay_array       { using type = Type; };
    template<class Type, unsigned N>   struct decay_array<Type[N]> { using type = Type*; };
    template<class Type>               struct decay_array<Type[]>  { using type = Type*; };

    template<class Type>   struct not_char_pointer              { static DOCTEST_CONSTEXPR value = 1; };
    ...
    template<class Type> struct can_use_op : public not_char_pointer<typename decay_array<Type>::type> {};
#endif // DOCTEST_CONFIG_TREAT_CHAR_STAR_AS_STRING
    // clang-format on
...
template <typename Type>
int registerExceptionTranslator(String (*)(Type)) {
    return 0;
}
...
using ticks_t = timer_large_integer::type;
    ...
    template <typename Type>
    using MultiLaneAtomic = Atomic<Type>;
